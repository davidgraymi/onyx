...
use crate::{ast::OnyxModule, generators::{CodeGenerator, CompileError}};
...
impl<W: Write> PythonGenerator<W> {
    ...
    fn write_line(&mut self, content: &str) -> io::Result<()> {
        let indent = " ".repeat(
            (self.current_indent_level * self.config.indent_spaces) as usize
        );
    ...
    pub fn generate_class(&mut self, name: &str, bases: &[&str]) -> io::Result<()> {
        ...
        self.write_line(&format!("{}A generated Python class.{}{}", quote, quote, "\n"))?;
